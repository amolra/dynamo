...
export function createEntireReactCode(
  ...
  const updateAppFile = (components: requestFields[]) => {
    const subToReturn = new BehaviorSubject<boolean>(false);
    const componentNametoPassArr: string[] = components.map((data: requestFields) =>
      capatalizeFirstLetterOfString(data.componentName)
    );
    const parentModuleNametoPassArr: string[] = components.map((data: requestFields) => data.parentModuleName);
    const newModuleNametoPassArr: string[] = components.map((data: requestFields) => data.newModuleName);
...
`;
  ...
  const createModulesObservable = createModules(fetTech, template, components[0].parentModuleName);

  createModulesObservable.subscribe((resultcreateModules: boolean) => {
    if (resultcreateModules) {
      console.log('Module created successfully');

      const createComponentFilesObservable = new BehaviorSubject<boolean>(false);
      ...
      for (const element of components) {
        ...
        createComponentFileObservable.subscribe((resultCreateComponentFile: boolean) => {
      ...
      createComponentFilesObservable.subscribe(() => {
