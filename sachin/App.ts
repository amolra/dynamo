...
// import { generateCode } from "./react-setup/generatecode/generatecode";
...
app.post("/project-setup", async (req, res) => {
  ...
  await createprojectStructure(fetTech).subscribe(async (result: boolean) => {
    if (result) {
      ...
      if (fetTech === "Angular") {
        ...
        //Code from Dynamo
        const responseModuleCreate = await createEntireReactCode(
          fetTech,
          template,
          req.body.component
        );
  ...
  //       const { parentModule, newModule } = req.body;
  //       const responseGenerateCode = await generateCode(
  //         parentModule,
  //         newModule,
  //         req.body.component
